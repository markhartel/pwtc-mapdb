        ...
        $(document).on( 'heartbeat-tick', function( e, data ) {
            if ( data['pwtc-refresh-post-lock'] ) {
                ...
                if ( received.lock_error ) {
                    $('#pwtc-mapdb-delete-map-div').html('<div class="callout small alert">You cannot delete post "<?php echo $map_title; ?>". ' + received.lock_error.text + '</div>');
                } 
...
<?php } else if ($attached_file) { ?>
    ...
    <?php if ($used_in_ride) { ?>
        <div class="callout small warning">
            <p>Warning, this route map is <a href="<?php echo $usage_link; ?>">used by ride templates or scheduled rides.</a> It can be deleted, but will then be unavailable to the ride.</p>
        </div>
    <?php } ?>
        <div class="callout">
            <div class="row column">
